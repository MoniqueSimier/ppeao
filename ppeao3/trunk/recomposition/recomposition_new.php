...
///////////////////////////////////////////////////////////////////////////////////////////////
		...
		if ($Wt == 0){ 
			...
			if ($rapport < 0.95000){
					...
					$info_deb[$key][$key2][9] = round($Nfdbq,0);
			...
			if (($rapport  >= 0.949999) && ($rapport  < 1.049999)){	
					...
					$info_deb[$key][$key2][5] = round($WfdbqI,2);		//Wt = somme(Wfdbq)
			}
...
print_debug("query_fraction_non_deb=".$query);
...
$result2 = pg_query($connection, $query2); //pg_close();
					...
					while($row2 = pg_fetch_row($result2))
						...
						if ($nb >= 5)
							...
							//$connection =pg_connect ("host=".$host." dbname=".$bdd." user=".$user." password=".$passwd);
								$query3=query_month($info_non_deb);
								print_debug($query3);
															...
															print_debug($query7);
																...
																while($row7 = pg_fetch_row($result7))
																	...
																	//	$connection =pg_connect ("host=".$host." dbname=".$bdd." user=".$user." password=".$passwd);
																		...
																		$query8=query_month($info_non_deb);
																			print_debug($query8);
																		$result8 = pg_query($connection, $query8);
																		//pg_close();
